...
public abstract class BasicPipe : MonoBehaviour
{
    ...
    public void renderLeak(int direction) {
        // create sprite
        SpriteRenderer createdLeakDisplay = (SpriteRenderer) UnityEngine.Object.Instantiate(LEAK_SPRITE_RENDERER_TEMPLATE);
        // modify sprite info
    ...
    public void Update() {
        ...
        if (targetRotationDir != 0) {
            ...
            if (rotationProgress > 1) {
                ...
                targetRotationDir = 0;
                PipeGrid.isRotating = false;
                PipeGrid.pendingFlowUpdate = true;
            }
            // update rotation
            handleSpriteRotation(this.transform, rotation, targetRotationDir, rotationProgress);
        }
