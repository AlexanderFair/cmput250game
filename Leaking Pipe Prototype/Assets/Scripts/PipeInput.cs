...
public class PipeInput : BasicPipe
{
    ...
    void Update()
    {
        ...
        if (PipeGrid.pendingFlowUpdate) {
            ...
            // reset leak amount
            PipeGrid.liquidDecrement = 0;


            if (PipeGrid.liquidRemaining > 0) {
            ...
            // handle liquid loss
            PipeGrid.liquidRemaining -= PipeGrid.liquidDecrement;
            // if the liquid is depleted, make sure it does not go negative
            if (PipeGrid.liquidRemaining <= 0) {
                PipeGrid.liquidRemaining = 0;
            }
            ...
            PipeGrid.pendingFlowUpdate = false;
        }
    }
